...
class SmithableEquipment {
    ...
    private static readonly RONZE_UNF_BOLTS = new SmithableEquipment("Bolts (unf)", 2349, 9375, 10, 1121, 4, 11461, 3, 1, 11459);
...
export class Bar {
    ...
    constructor(
        ...
        frame: number,
        buttons: number[][],
        items?: ImmutableSet<SmithableEquipment>
    ) {
        this.bar = bar;
        this.ores = ores;
    ...
    public static forBarId(barId: number): Optional<Bar> {
        return Optional.ofNullable(Bar.smeltables.get(barId));
    }
...
export class EquipmentMaking {
    ...
    */
    ...
    public static openInterface(player: Player): void {
        ...
        if (bar) {
            ...
            for (const b of bar.getItems()!) {
                ...
                player.getPacketSender().sendString(b.getBarFrame(), barColor + b.getBarsRequired().toString() + " " + (b.getBarsRequired() > 1 ? "bars" : "bar"));
    ...
    public static initialize(player: Player, itemId: number, interfaceId: number, slot: number, amount: number): void {
        for (const smithable of SmithableEquipment.values()) {
            if (smithable.getItemId() === itemId && smithable.getItemFrame() === interfaceId && smithable.getItemSlot() === slot) {
                player.getSkillManager().startSkillable(new ItemCreationSkillable(
                    ...
                    new Item(smithable.getItemId(), smithable.getAmount()), amount, new Optional<AnimationLoop>(new AnimationLoop(new Animation(898), 3)), smithable.getRequiredLevel(), 10, Skill.SMITHING));
                break;
            }
        }
    }
...
export class Smelting extends ItemCreationSkillable {
    ...
    */
    ...
    constructor(bar: Bar, amount: number) {
        super(bar.getOres().map(ore => new RequiredItem(ore)), new Item(bar.getBar()), amount,
            Optional.of(new AnimationLoop(this.ANIMATION, 4)), bar.getLevelReq(), bar.getXpReward(), Skill.SMITHING);
        this.bar = bar;
    }
