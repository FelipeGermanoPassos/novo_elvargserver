...
import { CombatPoisonData, Poison } from "../../task/impl/CombatPoisonEffect";
        ...
        if (attacker.isPlayer()) {
            ...
            if (damage > 0 && Math.floor(Math.random() * 20) <= 5) { // 1/4
                ...
                if (combatType === CombatType.MELEE || p_.getWeapon() === WeaponInterfaces.DART
                    ...
                    poison = Poison.getPoisonType(p_.getEquipment().get(Equipment.WEAPON_SLOT));
                } else if (combatType === CombatType.RANGED) {
                    isRanged = true;
                    poison = Poison.getPoisonType(p_.getEquipment().get(Equipment.AMMUNITION_SLOT));
                }
    ...
    */
    ...
    private static handleRedemption(attacker: Mobile, victim: Player, damage: number) {
        if ((victim.getHitpoints() - damage) <= (victim.getSkillManager().getMaxLevel(Skill.HITPOINTS) / 10)) {
            ...
            victim.getSkillManager().setCurrentLevels(Skills.PRAYER, 0);
            victim.getSkillManager().setCurrentLevels(Skills.HITPOINTS, victim.getHitpoints() + amountToHeal);
