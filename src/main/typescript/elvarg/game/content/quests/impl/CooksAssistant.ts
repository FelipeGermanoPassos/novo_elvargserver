...
class CooksAssistant implements Quest {
    ...
    firstClickNpc(player: Player, npc: NPC) {
        ...
        switch (COOKS_ASSISTANT.getProgress(player)) {
            case 'NOT_STARTED':
                ...
                break;
            case 1:
                player.getDialogueManager().start(this.dialogueBuilder, 20);
                break;
            case 2:
    ...
    private buildDialogues() {
        ...
        this.dialogueBuilder.add(
            ...
            new OptionsDialogue(1, new Map<string, (player: Player) => void>([
                ["What's wrong?", (player) => player.getDialogueManager().start(2)],
                ["Can you cook me a cake?", (player) => player.getDialogueManager().start(14)],
                ["You don't look very happy.", (player) => player.getDialogueManager().start(2)],
                ["Nice hat.", (player) => player.getDialogueManager().start(17)]
            ])),
            ...
            new OptionsDialogue(5, new Map<string, (player: Player) => void>([
