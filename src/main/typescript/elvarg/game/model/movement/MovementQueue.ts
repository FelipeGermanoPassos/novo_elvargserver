...
export class MovementQueue {
    ...
    public getMobility(): Mobility {
        ...
        if (this.player != null) {
            ...
            if (playerDueling.inDuel() && playerDueling.getRules()[DuelRule.NO_MOVEMENT.forId(0)]) {
    ...
    public processCombatFollowing() {
        ...
        if (!dancing) {
            ...
            let destination = PathFinder.getClosestAttackableTile(this.character, following, attackDistance);
            if (destination == null) {
                return;
            }
        }
