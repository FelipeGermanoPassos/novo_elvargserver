export class WildernessArea extends Area {
    ...
    constructor() {
    super([
    new Boundary(2940, 3392, 3525, 3968),
    ...
    if (character.isPlayer()) {
    const player = character.getAsPlayer();
    player.setWildernessLevel(this.getLevel(player.getLocation().getY()));
    ...
    canAttack(attacker: Mobile, target: Mobile): CanAttackResponse {
    ...
    onPlayerRightClick(player: Player, rightClicked: Player, option: number): void {
    }
    
    defeated(player: Player, character: Mobile): void {
        if (character.isPlayer()) {
            BountyHunter.onDeath(player, character.getAsPlayer(), true, 50);
        }
    }
    
    overridesNpcAggressionTolerance(player: Player, npcId: number): boolean {
        return true;
    }
