        ...
        player.getMovementQueue().walkToObject(object, () => {
            switch (object.getId()) {
                ...
                case ObjectIdentifiers.FIRE_23: //Barb village fire
                    ...
                    if (cookable) {
                        player.getPacketSender().sendCreationMenu(new CreationMenu("How many would you like to cook?", Arrays.asList(cookable.getcookedItem()), (productId, amount) => {
                    ...
                    if (object.getId() == ObjectIdentifiers.FIRE_5) {
                        ...
                        if (log) {
                            player.getPacketSender().sendCreationMenu(new CreationMenu("How many would you like to burn?", Arrays.asList(log.getLogId()), (productId, amount) => {
                                player.getSkillManager().startSkillable(new Firemaking(log, object, amount));
                            }));
                            return;
                        }
                    }
