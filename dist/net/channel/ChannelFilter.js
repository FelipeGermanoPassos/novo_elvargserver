...
var ChannelFilter = /** @class */ (function () {
    function ChannelFilter() {
        ...
        this.server = new ws_1.WebSocketServer({ port: 8080 });
        this.server.on('connection', function (socket) {
            _this.onConnection(socket);
        });
    }
    ChannelFilter.prototype.onConnection = function (socket) {
        var host = ByteBufUtils_1.ByteBufUtils.getHost(socket._socket);
        ...
        if (this.connections.count(host) > NetworkConstants_1.NetworkConstants.CONNECTION_LIMIT) {
            LoginDecoder_1.LoginDecoder.sendLoginResponse(socket, LoginResponses_1.LoginResponses.LOGIN_CONNECTION_LIMIT);
            return;
        }
        //CHECK BANS
