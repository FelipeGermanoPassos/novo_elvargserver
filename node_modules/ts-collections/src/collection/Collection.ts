...
export const INITIAL_CAPACITY:number = Utils.isDefined(window) && window[$$TS_COLLECTIONS_INITIAL_CAPACITY]
    ? parseInt(window[$$TS_COLLECTIONS_INITIAL_CAPACITY])
    : 10000;

export const CAPACITY_GROW_FACTOR:number = Utils.isDefined(window) && window[$$TS_COLLECTIONS_CAPACITY_GROW_FACTOR]
...
export abstract class Collection<TItem> extends AbstractCollection<TItem> {
    ...
    private isSuitable(value:TItem, predicate:IPredicate<TItem>|IPredicateFunction<TItem>):boolean {
        return Utils.isFunction(predicate)
